...
| `ENG-15` | 中 | 纵深排序与代码影子 | **机器验收通过，待作者实机看，未提交。这是纵深第一次能用眼睛看。** 排序算在规则层（两个键：纵深为主、脚底为次，`y_sort` 只排一个键所以不用它），引擎层只写 `z_index`；影子由代码画、随高度只缩小不变淡（项目禁半透明）；碰撞地面对应带中线，地形按 48px 带画。新入口 `depth_dev.py` 13/13（含截图数像素证明前后关系真的换了）、自证 37/37，verify 6/6 与 295/295，另两个探针无回归。详见 [`issue-ENG-15`](./issue-ENG-15-depth-sorting-shadow.md) |
