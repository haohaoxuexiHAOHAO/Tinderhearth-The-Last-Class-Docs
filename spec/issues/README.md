...
| `GP-12` | 中 | 引擎层：主角节点驱动 + 动画接入 | **机器验收已通过，待作者手感确认。** 主角驱动与缺图几何占位已落地；verify 6/6、259/259，输入17/17。2026-09-08 换用 `ART-6` 自绘七表，图形探针扩到 41/41（含逐帧精灵相位、跨动作脚底行、本体≤32px 与两条前提判据）。详见 [`issue-GP-12`](./issue-GP-12-player-actor-engine.md) |
...
| `GP-15` | 高 | 规则层：纵深轴运动（连续、48px 带、三轴分离） | **下一件。** 依赖 `GP-10`（已完成）——这是**对已实现单平面模型的改造**，不是新建。作者 2026-09-09 把战斗关卡定为带连续可行走纵深的横版（belt-scroll），正典见[战斗与关卡 · 空间模型](../../canon/gameplay/战斗与关卡.md)。详见 [`issue-GP-15`](./issue-GP-15-depth-axis-motion.md) |
