...
| `GP-13` | 中 | 引擎层：命中检测 + 打击反馈四件套 + 木桩 | **机器验收通过，待作者手感确认，未提交。** 开关震屏各 62/62、日志自证、verify 6/6 与 259/259；独立 HitFeedbackDev 体验入口，未推进 GP-14/ENG-6。2026-09-08 判定框按轻重分开并加行为级判据（同距离轻击打空、重击打到）。详见 [`issue-GP-13`](./issue-GP-13-hit-detection-feedback.md) |
| `GP-15` | 高 | 规则层：纵深轴运动（连续、48px 带、三轴分离） | **机器验收通过，未提交；手感确认不在本条** —— 纵深在画面上还看不见（引擎层一次都没读它去改绘制，那是 `ENG-15`），实机判断要等 `ENG-15` 接上排序与影子之后。三轴分离落地：纵深连续、钳在新增 `DepthBand` 的 48px 带内、离地锁落地解锁、出招三轴一同定身、闪避方向改二维。纵深行走 60px/s 与纵深闪避 90px/s 是未校准初值归 `GP-6`。verify 6/6、283/283（原 259），图形探针 45/45（原 41，新增 4 条纵深接线判据）、GP-13 两轮 63/63、输入 17/17。纵深的绘制与命中仍归 `ENG-15`／`GP-16`。详见 [`issue-GP-15`](./issue-GP-15-depth-axis-motion.md) |
...
| `ART-6` | 中 | [主角动作帧接仓与闪白口径](./issue-ART-6-role-action-frames.md) | **机器验收通过，两条作者待办未收，未提交。** 74 张自绘逐帧件接成 10 表（帧框 46×32 实测算出）；闪白定为由代码持有、进仓表禁单色帧；帧框与判定框常量绑定登记表，判定框按轻重分开（轻 18×4、重 22×16，由 Active 帧实测导出，仍归 `GP-6`）。待作者：参考线逐帧一致、`hit` 两帧重画、实机确认；发行前替换正式美术 |
...
| id | 优先级 | 待办 | 备注 |
| --- | --- | --- | --- |
| `ENG-14` | 高 | [输入专项探针互不污染](./issue-ENG-14-input-probe-isolation.md) | **输入修复已验证，待收口。** GP-12 前置闸门已通过；串行共享输入探针，原输入 17/17，自证 10/10，HUD 28/28，verify 6/6。证据及限制见条目；未提交 |
| `ENG-15` | 中 | 纵深排序与代码影子 | 依赖 `GP-15`、`GP-12`。纵深靠后先画、同纵深按脚底排，排序与命中共用同一份纵深值；影子由代码持有、不进精灵帧，跳跃时留在地面作高度提示。详见 [`issue-ENG-15`](./issue-ENG-15-depth-sorting-shadow.md) |
