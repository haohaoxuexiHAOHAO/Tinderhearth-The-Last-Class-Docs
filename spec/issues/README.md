...
| `ENG-15` | 中 | 纵深排序与代码影子 | **机器验收通过，待作者实机看，未提交。这是纵深第一次能用眼睛看。** 排序算在规则层（两个键：纵深为主、脚底为次，`y_sort` 只排一个键所以不用它），引擎层只写 `z_index`；影子由代码画、随高度只缩小不变淡（项目禁半透明）；碰撞地面对应带中线，地形按 48px 带画。影子宽度按作者裁定跟当前帧的本体宽走（有实体宽下限，否则走动时手臂摆动会让它以 15Hz 缩到一半）。新入口 `depth_dev.py` 15/15（含截图数像素证明前后关系真的换了）、自证 41/41，verify 6/6 与 299/299，另两个探针无回归。详见 [`issue-ENG-15`](./issue-ENG-15-depth-sorting-shadow.md) |
