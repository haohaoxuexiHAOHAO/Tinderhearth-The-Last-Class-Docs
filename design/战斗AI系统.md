...
本页回答一件事：**一个不由玩家操作的角色，这一帧决定做什么。** 敌人与队友走**同一条决策循环**，差别只在挂上去的门与可用的动作 —— 敌人那一侧挂警戒状态机与警戒传播，队友那一侧挂两个行为档号与一个指令档。

**它持有**：决策循环那条链、敌人的警戒状态机与脱战、警戒传播、预警与对策的声明、分级面板怎么取子集、队友两条轴各自的行为表与分轴判据、指令档与它的默认档、难度那两个量作用在哪。

**它不持有**：可见性数据（[`感知与视野系统`](./感知与视野系统.md)）；姿态与流转（[`角色动作状态系统`](./角色动作状态系统.md)）；伤害与状态的结算（[`伤害系统`](./伤害系统.md)）；技能的字段与冷却（[`技能系统`](./技能系统.md)）；有哪些敌人（内容，`ENG-5`）；敌人预算与放置位（[`关卡系统`](./关卡系统.md)）；寻路算法；经营侧的学生日程 AI。
...
- **不做界面**：两个档号怎么看得见、指令怎么下、打断反馈长什么样归界面侧与作者实机定（`UI-17`）。
- **不做敌人的动画、碰撞区域与摆位**：作者在 Godot 里管（[ADR-0009](../decisions/ADR-0009-编辑器主导的开发模式.md)）。
- **不设计存档序列化格式**：只定「关卡内的警戒状态进临时续玩点，两个档号是派生量所以不单独存」。
- **同名不同物，两处都要写全名**：
...
**必须实机确认项**：低档与高档队友在画面上分不分得出来（**这是本设计的核心风险，而且永远没有机器判据** —— 机器只能判「这一档比上一档多了一条行为」）；预警动作看不看得出来是预警；打断敌方施法的反馈与命中反馈分不分得开；敌人被叫来的那一下读不读得出「它是被叫来的」；队友补位看起来是配合还是乱走；指令面板看不看得出现在是哪一档。按 [ADR-0009](../decisions/ADR-0009-编辑器主导的开发模式.md) 的分工由作者实机看，本页不判。
...
- **代码**：决策循环与两个档号落**规则层**（它只吃集合、面板与配置，不需要引擎）；敌人节点、动画与预警表现落引擎层。相邻的既有件是 `rules/Combat/StatusEffects.cs`（难度那两个量要落它的永久项，**而永久项那一档要等 `GP-18` 的载体扩容**）、`rules/Combat/ComboStateMachine.cs` 与 `rules/Combat/MotorState.cs`（动作机那一侧，本页只往它递意图）。**敌人要作者搭，所以落地排在 `ENG-21` 之后或与它同批。**
