...
##
## 它要回答的只有一个问题：**中文在实机上看不看得清。**
## 那件事没有机器判据 —— 覆盖率与字宽量得出来，可读性只能把真字渲出来用眼睛看。
## 判过哪些字号与分辨率、结论是什么，见 ADR-0008 与待办台账，本文件不复述。
##
## 两种用法：
...
const SPEAKER := "教官 · 芜"
const DIALOGUE := "明天出征前，把这批药剂分给三名学生 —— 别让谁空着手上前线。回来时我要看到四个人，不是三个。"
